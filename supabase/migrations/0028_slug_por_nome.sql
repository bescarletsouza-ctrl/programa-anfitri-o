...
-- Passa a ser "nome-do-anfitriao-xxxx" (xxxx = 4 caracteres pra evitar
-- colisão, já que o slug é único no banco). A aplicação já gera slugs nesse
-- formato para anfitriões novos; esta migração atualiza os que já existem.
--
...
with base as (
  select id, coalesce(nullif(public.slugify_nome(nome), ''), 'convidado') as raiz,
